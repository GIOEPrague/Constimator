...
class OpenmartControllerEsticalc extends JControllerForm
{
    public function __construct($config = array()) {
        ...
        JLog::addLogger( array('text_file' => 'com_openmart.log'), JLog::INFO );
    ...
    public function calculateObject(){
        ...
        JLog::add('calculateObject() / $currency: '.print_r($currency,true));
        $sObject[$id] = $object[$id];
        if($sObject[$id]['categoryId'] > 0 && $sObject[$id]['paymentZoneId'] > 0){
            if($sObject[$id]['currency'] != $currency){
                JLog::add('calculateObject() / amount: '.print_r($sObject[$id]['materialCost'],true));
                JLog::add('calculateObject() / from: '.print_r($sObject[$id]['currency'],true));
                JLog::add('calculateObject() / to: '.print_r($currency,true));
                $sObject[$id]['materialCost'] = $this->getCurrencyData($this->getGoogleCurrencyUrl($sObject[$id]['materialCost'], $sObject[$id]['currency'], $currency));
                
                JLog::add('calculateObject() / $sObject[$id][materialCost]: '.print_r($sObject[$id]['materialCost'],true));
            }
            ...
            JLog::add('calculateObject() / $sObject[$id][currency]: '.print_r($sObject[$id]['currency'],true));
    ...
    public function getPrice(){
        ...
        $newPrice = $this->getCurrencyData($this->getGoogleCurrencyUrl($amount, $from, $to));
        echo $newPrice;
        $app->close();
